...
##########################################################################
class TrackingMemoryBank(nn.Module):
    """记忆演化模块"""
    def __init__(self, capacity=100, merge_threshold=0.8):
        super().__init__()
        self.memory = []
        self.capacity = capacity
        self.threshold = merge_threshold

    def add(self, prompt, features):
        if len(self.memory) >= self.capacity:
            self.memory.pop(0)
        self.memory.append({"prompt": prompt.detach(), "features": features.detach()})

    def merge_prompts(self, current_prompt, current_feat):
        merged = current_prompt.clone()
        if len(self.memory) == 0:
            return merged

        # 计算相似度
        sims = [F.cosine_similarity(current_feat.flatten(), m["features"].flatten(),dim=0)
                for m in self.memory]
        max_sim_idx = torch.argmax(torch.tensor(sims))

        if sims[max_sim_idx] > self.threshold:
            # 合并提示参数
            merged = 0.7 * merged + 0.3 * self.memory[max_sim_idx]["prompt"]
        return merged
        # # Forward head
        # # 记忆演化
        # current_feat = x.mean(dim=1)  # 全局特征
        # merged_prompt = self.memory_bank.merge_prompts(
        #     aux_dict["prompt"], current_feat)
        #
        # # 更新记忆库
        # self.memory_bank.add(merged_prompt, current_feat)
class PromptMLP(nn.Module):
    ...
    def forward(self, x):
        ...
        return out.view(bsz, self.length, self.in_dim)  # 重塑为 [bsz, length, embed_dim]

class ContinuousPrompt(nn.Module):
    """分层提示记忆库"""
    def __init__(self, num_layers=12, embed_dim=768, prompt_length=4):
        super().__init__()
    ...
    def forward(self, x):
        ...
        for module in self.memory:
            outputs.append(module(x).unsqueeze(1))  # [bsz, 1, len, dim]
        return torch.cat(outputs, dim=1)  # [bsz, layers, len, dim]
...
class iTPN(nn.Module):
    def __init__(self, search_size=224, template_size=112, patch_size=16, in_chans=3, num_classes=1000, fpn_dim=256, fpn_depth=2,
        ...
        prompt_length = 4  ############
        #######################################################################################
        # 初始化动态提示模块，使用self.embed_dim
        self.prompt_memory = ContinuousPrompt(
            num_layers=depth,
            embed_dim=self.embed_dim,  # 使用self.embed_dim确保维度匹配
            prompt_length=prompt_length
        )
        self.prompt_length = prompt_length
        self.memory_bank = TrackingMemoryBank()  #######################
    ...
    def forward_features(self, template_list, search_list,template_anno_list):
        ...
        #     query_embed = torch.cat([cls_tokens, x[:, 0:1]], dim=1).mean(dim=1)  # 融合模板特征

        # 动态生成分层提示
        batched_prompt = self.prompt_memory(xz)  # [B, L, plen, D]
        # self.memory_bank.add(batched_prompt, x.mean(dim=1))
        #######################################################################################3
