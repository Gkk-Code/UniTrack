...
class GlobalPromptMemory(nn.Module):

    loss1 = 0
    def __init__(self, capacity=100, num_layers=12, prompt_len=5, embed_dim=768):
        super().__init__()
        ...
        self.fuse_mlp = nn.Sequential(
            nn.Linear(num_layers * prompt_len * embed_dim, embed_dim * 2),
            nn.ReLU(inplace=True),
            nn.Linear(embed_dim * 2, num_layers * prompt_len * embed_dim)
        )
        ...
        # Reduce + Expand
        self.reduce = nn.Linear(embed_dim, embed_dim // 2)
        self.expand = nn.Linear(embed_dim, embed_dim)


    def add(self, batched_prompt):  # [B, L, plen, D]
        if len(self.memory) >= self.capacity:
            self.memory.pop(0)
        self.memory.append(batched_prompt.detach())

    def average_memory(self):  # → [B, L, plen, D]
        if len(self.memory) == 0:
            return None
        return torch.mean(torch.stack(self.memory), dim=0)

    def compute_metric_loss(self, p_b):



        if len(self.memory) < 2:

            self.metric_loss_value = 0.0

            return 0

        # flatten prompts for distance measure
        # [B, L, plen, D] → [B, L*plen*D]
        p_flat = p_b.reshape(p_b.size(0), -1)


        distances = []
        for h in self.memory:
            h_flat = h.reshape(h.size(0), -1)
            dist = torch.norm(p_flat - h_flat, p=2, dim=1)  # [B]
            distances.append(dist)

        dist_tensor = torch.stack(distances, dim=0)


        nearest_idx = torch.argmin(dist_tensor, dim=0)  # [B]
        farthest_idx = torch.argmax(dist_tensor, dim=0)  # [B]


        B = p_b.size(0)
        nearest_prompt = torch.stack([self.memory[nearest_idx[i]][i] for i in range(B)], dim=0)
        farthest_prompt = torch.stack([self.memory[farthest_idx[i]][i] for i in range(B)], dim=0)

        # flatten
        nearest_flat = nearest_prompt.reshape(B, -1)
        farthest_flat = farthest_prompt.reshape(B, -1)


        pos_dist = torch.norm(p_flat - nearest_flat, p=2, dim=1).pow(2)
        neg_dist = torch.norm(p_flat - farthest_flat, p=2, dim=1).pow(2)

        raw_loss = pos_dist - neg_dist
        metric_loss = torch.clamp(raw_loss.abs(), min=0).mean()


        self.metric_loss_value = metric_loss



        GlobalPromptMemory.loss1 = metric_loss

        return metric_loss


    def merge_all_prompts(self, current_batched_prompt):

        B, L, plen, D = current_batched_prompt.shape

        hist_avg = self.average_memory()  # [B, L, plen, D]
        self.compute_metric_loss(current_batched_prompt)


        hist_flat = hist_avg.view(B, -1)
        fused_hist = self.fuse_mlp(hist_flat).view(B, L, plen, D)

        output_prompts = []
        for i in range(L):
            cur = current_batched_prompt[:, i]       # [B, plen, D]
            fused = fused_hist[:, i]                 # [B, plen, D]
            ...
            fused_attn = torch.matmul(attn, V)

            cur_red = self.reduce(cur)
            fused_red = self.reduce(fused_attn)
            concat = torch.cat([cur_red, fused_red], dim=-1)
            final = self.expand(concat)

            output_prompts.append(final)
...
class PromptMLP(nn.Module):
    ...
    def __init__(self, in_dim=768, hidden_dim=64, length=4):
        super().__init__()
        self.in_dim = in_dim
        self.length = length
        self.block = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Linear(hidden_dim, in_dim * length)
        )

    def forward(self, x):
        bsz = x.size(0)
        out = self.block(x)  # [bsz, embed_dim*length]
        return out.view(bsz, self.length, self.in_dim)  # [bsz, length, embed_dim]


class ContinuousPrompt(nn.Module):

    def __init__(self, num_layers=12, embed_dim=768, prompt_length=1):
        super().__init__()
        self.num_layers = num_layers
        self.memory = nn.ModuleList([
            PromptMLP(embed_dim, hidden_dim=64, length=prompt_length)
    ...
    def forward(self, x):
        ...
        for module in self.memory:
            outputs.append(module(x).unsqueeze(1))  # [bsz, 1, length, dim]
        return torch.cat(outputs, dim=1)  # [bsz, layers, length, dim]
...
class Fast_iTPN(nn.Module):
    def __init__(self, search_size=224,template_size=112, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        prompt_length = 5
        self.prompt_memory = ContinuousPrompt(
            num_layers=depth,
        ...
        self.memory_bank = GlobalPromptMemory(
            capacity=100,
            num_layers=depth,
    ...
    def forward_features(self, template_list, search_list,template_anno_list, text_src, task_index):
        ...
        with torch.no_grad():
            cls_tokens = self.cls_token.expand(B, -1, -1)
            query_embed = torch.cat([cls_tokens, xz[:, 0:1]], dim=1).mean(dim=1)  # 融合模板特征


        batched_prompt = self.prompt_memory(query_embed)  # [B, L, plen, D]
        self.memory_bank.add(batched_prompt)
        fused_prompts = self.memory_bank.merge_all_prompts(batched_prompt)  # List of [B, plen, D]
        avg_prompt = torch.mean(torch.stack(fused_prompts, dim=1), dim=1)  # [B, plen, D]
        xz = torch.cat([avg_prompt, xz], dim=1)

        # xz = torch.cat([prompt, xz], dim=1)
