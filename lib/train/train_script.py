...
# network related
from lib.models.UniTrack import build_UniTrack
from lib.train.actors import UniTrack_Actor
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "UniTrack":
        net = build_UniTrack(cfg)
    else:
    ...
    if  settings.script_name == "UniTrack":
        ...
        actor = UniTrack_Actor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
